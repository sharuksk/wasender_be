...
console.log(date.toISOString());
  ...
  let getData= await db.collection("scheduled_messages").find({status: "pending", date: date.toISOString()}).sort({date: 1}).toArray();
  ...
  if (getData[0]){
      try {
        for (let i = 0; i < getData.length; i++) {
          ...
          await db.collection("scheduled_messages").findOneAndUpdate({_id: getData[i]._id},{$set: {status: "sent"}})
